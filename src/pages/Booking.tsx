...
import { fetchRawatJalan, fetchRiwayatPerawatan, fetchRiwayatKunjungan, RawatJalanParams } from '@/api/rawatJalan';
import { PatientDetail } from '@/components/rawat-jalan/PatientDetail';
import { DataTable, Column } from '@/components/DataTable';
import { Input } from '@/components/ui/input';
import { Button } from '@/components/ui/button';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/components/ui/select';
import { Users, Search, Filter, X, Clock, Calendar as CalendarIcon, ArrowRight, List } from 'lucide-react';
...
const Booking = () => {
  ...
  const [date, setDate] = useState<DateRange | undefined>({
    from: addDays(new Date(), 1), // Default tomorrow
  ...
  const formattedDateFrom = date?.from ? format(date.from, 'yyyy-MM-dd') : format(addDays(new Date(), 1), 'yyyy-MM-dd');
  ...
  if (activeTab === 'pasien-lanjutan') {
    ...
    // In booking context, "lanjutan" logic might be different or not applicable, but keeping structure for consistency
    // If booking logic requires "future" dates, then "pasien-lanjutan" (past) might not make sense here.
    // However, if we follow "RawatJalan" logic:
    const yesterday = new Date(today);
    yesterday.setDate(yesterday.getDate() - 1);
    
    // ... (rest of logic)
    
    // BUT for Booking, usually we look forward. 
    // If the instruction implies EXACT consistency with Rawat Jalan tabs:
    // We keep it as is. 
    
    // Check if current date selection is just "today" (default state) - here default is tomorrow
    const tomorrow = addDays(new Date(), 1);
    const isDefaultTomorrow = date?.from && date?.to && 
                           format(date.from, 'yyyy-MM-dd') === format(tomorrow, 'yyyy-MM-dd') && 
                           format(date.to, 'yyyy-MM-dd') === format(tomorrow, 'yyyy-MM-dd');
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['rawat-jalan', params],
    queryFn: () => fetchRawatJalan(params),
  });
    ...
    <div className="space-y-6">
      ...
      <div>
        <div className="flex items-center gap-2 mb-1">
          ...
          <h1 className="text-xl font-bold text-foreground">Booking Pasien</h1>
        </div>
        <p className="text-sm text-muted-foreground">
          Daftar pasien yang telah melakukan booking untuk pelayanan medis mendatang
        </p>
      </div>
      ...
      <div className="bg-card rounded-xl border p-6 shadow-sm space-y-4">
        {!activePatientTab ? (
          <>
            <h2 className="text-base font-bold text-foreground">Daftar Pasien Hari Ini</h2>
