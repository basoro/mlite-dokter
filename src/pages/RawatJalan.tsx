...
const tabs = [
  ...
  { key: 'sesi-pagi', label: 'Sesi Pagi', icon: CalendarIcon },
  { key: 'sesi-sore', label: 'Sesi Sore', icon: CalendarIcon },
  { key: 'rujukan-internal', label: 'Rujukan Internal', icon: ArrowRight },
...
const RawatJalan = () => {
  ...
  const { data: riwayatData, isLoading: isLoadingRiwayat } = useQuery({
    ...
    queryFn: () => {
      ...
      // Encode no_rawat to handle slashes (e.g., 2026/02/26/000001 -> 2026%2F02%2F26%2F000001)
      const encodedNoRawat = encodeURIComponent(selectedPatient.no_rawat);
      return fetchRiwayatPerawatan(selectedPatient.no_rkm_medis, encodedNoRawat);
    },
    ...
    <div className="space-y-6">
      ...
      <div>
        <div className="flex items-center gap-2 mb-1">
          ...
          <h1 className="text-xl font-bold text-foreground">Pasien Rawat Jalan</h1>
        </div>
        <p className="text-sm text-muted-foreground">
          Pasien yang mendapatkan pelayanan medis tanpa harus menginap di rumah sakit
        </p>
      </div>
