...
import { fetchRawatInap, fetchRiwayatPerawatan, fetchRiwayatKunjungan, RawatJalanParams } from '@/api/rawatJalan';
import { PatientDetail } from '@/components/rawat-jalan/PatientDetail';
import { DataTable, Column } from '@/components/DataTable';
import { Input } from '@/components/ui/input';
import { Button } from '@/components/ui/button';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/components/ui/select';
import { Users, Search, Filter, X, Clock, Calendar as CalendarIcon, BedDouble } from 'lucide-react';
import { format, addDays, differenceInYears, differenceInMonths, differenceInDays } from 'date-fns';
...
const tabs = [
  { key: 'hari-ini', label: 'Data Pasien', icon: Clock },
  { key: 'masuk', label: 'Masuk', icon: CalendarIcon },
  { key: 'pulang', label: 'Pulang', icon: CalendarIcon },
];
...
const RawatInap = () => {
  ...
  let effectiveStatus = status !== 'semua' ? status : undefined;
    ...
    if (activeTab === 'pasien-lanjutan') {
         ...
         // Standard logic for other tabs (hari-ini, masuk, pulang)
         // Default is today if no date selected
         // But for 'Masuk' and 'Pulang' tabs, we might want to respect the date picker strictly
         // Or default to today.
         
         // Current logic already handles formattedDateFrom/To based on date state or today default.
    }
  ...
  const params: RawatJalanParams = {
    ...
    search: debounced,
    filter: activeTab,
    status: activeTab === 'pasien-lanjutan' ? 'belum' : effectiveStatus,
    status_bayar: statusBayar !== 'semua' ? statusBayar : undefined,
    rujukan_internal: activeTab === 'rujukan-internal' ? true : undefined,
  };

  const { data, isLoading } = useQuery({
    queryKey: ['rawat-inap', params],
    queryFn: async () => {
      const res = await fetchRawatInap(params);
      console.log('Rawat Inap Response:', JSON.stringify(res, null, 2));
      return res;
    },
  });
  ...
  const columns: Column<Record<string, unknown>>[] = [
    ...
    { 
      ...
      label: 'Umur',
      render: (item) => {
        if (item.umur && item.umur !== '0' && item.umur !== '-') return <span>{String(item.umur)}</span>;
        
        if (item.tgl_lahir) {
          const birthDate = new Date(item.tgl_lahir as string);
          ...
          let ageString = '';
          if (years > 0) ageString = `${years} Th`;
          else if (months > 0) ageString = `${months} Bl`;
          else ageString = `${days} Hr`;
    ...
    <div className="space-y-6">
      ...
      <div>
        <div className="flex items-center gap-2 mb-1">
          ...
          <h1 className="text-xl font-bold text-foreground">Pasien Rawat Inap</h1>
        </div>
        <p className="text-sm text-muted-foreground">
          Pasien yang sedang menjalani perawatan inap di rumah sakit
        </p>
      </div>
      ...
      <div className="bg-card rounded-xl border p-6 shadow-sm space-y-4">
        {!activePatientTab ? (
          <>
            <h2 className="text-base font-bold text-foreground">Daftar Pasien Rawat Inap</h2>
            ...
            <DataTable 
              ...
              data={data?.data || []} 
              isLoading={isLoading} 
              emptyMessage="Tidak ada data Rawat Inap"
              onRowClick={(row) => {
